...
const VideoUploader: React.FC = () => {
  ...
  const checkCloudinaryConfig = async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : 
               (axios.isAxiosError(err) ? err.response?.data?.message || 'Failed to check Cloudinary configuration' : 
               'Failed to check Cloudinary configuration'));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(axios.isAxiosError(err) ? err.response?.data?.message || 'Video upload failed' : 'Video upload failed');
      setUploadResult(null);
    } finally {
      setUploading(false);
    }
